"""Publish the cache consistency / penetration-breakdown-avalanche article.

Usage:
    python tools/publish_cache_consistency_article.py
    python tools/publish_cache_consistency_article.py --cover <url>
    python tools/publish_cache_consistency_article.py --skip-cover-check
...
of existing tags (redis / cache / backend-dev / performance).
"""
...
ARTICLE_EXCERPT = (
    ...
    "到穿透/击穿/雪崩三兄弟的布隆过滤器、互斥重建、TTL 抖动，"
    "附一段可复现的并发实测：1500 次穿透请求从 1500 次回源压到 0，"
    "300 并发击穿从 183 次回源压到 1，雪崩峰值从 1000 削到 12。"
)
ARTICLE_COVER = (
