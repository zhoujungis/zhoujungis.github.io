"""Publish the LC 234 (palindrome linked list) article — the seventh algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc234_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc234_article.py
...
Source Markdown: ``article/lc-234-palindrome-linked-list.md``.

Same guard set as publish_lc19_article.py (adapted, not shared, so each
...
- viz check: the placeholder must survive markdown + bleach —
  ``.algo-viz--lc234`` (three-stage palindrome walkthrough).
- number check: headline figures must be present in the text. The three-step
  decomposition is the core of the article; if the prose and the
  implementation (frontend/src/viz/palindromeSteps.js) drift apart, this guard
  is what catches it.
...
SOURCE = (
    ...
    / "article"
    / "lc-234-palindrome-linked-list.md"
)

ARTICLE_TITLE = "【LC 234】回文链表：把后半段翻过来，它应该跟前半段长得一模一样"
ARTICLE_SLUG = "lc-234-palindrome-linked-list"
ARTICLE_EXCERPT = (
    "回文链表 = 找中点 + 反转后半段 + 逐对比较，三步分别是 LC 876、LC 206 和数组双指针的子问题。"
    "本文说清中点为什么归后半段、为什么不像重排链表那样需要断开、比较循环为什么以 while p2 为准，"
    "并附可回放的三阶段推演动画与边界推演表。"
)
# 自制封面（tools/make_lc234_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-234-cover.png"
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc234"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-234-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
