"""Publish the LC 236 (lowest common ancestor of a binary tree) article — the eleventh algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc236_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc236_article.py
...
Source Markdown: ``article/lc-236-lowest-common-ancestor-of-a-binary-tree.md``.

Same guard set as publish_lc102_article.py (adapted, not shared, so each
...
- viz check: the placeholder must survive markdown + bleach —
  ``.algo-viz--lc236`` (tree + explicit call stack + return-value conveyor).
- number check: headline figures must be present in the text. The three
  return-value identities (null / p-or-q-as-messenger / the answer itself) and
  the fork-point criterion are the spine of the article; if the prose and the
  state machine (frontend/src/viz/lcaSteps.js) drift apart, this guard catches it.
...
ARTICLE_TITLE = "【LC 236】二叉树的最近公共祖先：为什么「命中即返回」就自动处理了「自己是祖先」"
ARTICLE_SLUG = "lc-236-lowest-common-ancestor-of-a-binary-tree"
ARTICLE_EXCERPT = (
    "LC 236 只有 8 行代码，但它考的是「递归返回值能承载几种含义」。"
    "本文用一个只问「子树里有没有」的后序 DFS，把返回值拆成三种身份："
    "null、作为信使的 p/q、以及答案本身，并说明为什么"
    "「一个节点也可以是它自己的祖先」靠 return 那一行就自动处理了。"
    "附可回放的树 + 调用栈 + 返回值传送带三段式推演动画、23 帧完整推演表，"
    "以及 235 / 1650 / 1676 / 1123 四个变体的改法。"
)
# 自制封面（tools/make_lc236_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-236-cover.png"
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc236"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-236-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
