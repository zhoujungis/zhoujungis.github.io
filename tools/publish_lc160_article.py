"""Publish the LC 160 (intersection of two linked lists) article — the eighth algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc160_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc160_article.py
...
Source Markdown: ``article/lc-160-intersection-of-two-linked-lists.md``.
...
- viz check: the placeholder must survive markdown + bleach —
  ``.algo-viz--lc160`` (two-cursor crossing walkthrough).
- number check: headline figures must be present in the text. The
  ``a + c + b == b + c + a`` argument is the core of the article; if the prose
  and the implementation (frontend/src/viz/intersectSteps.js) drift apart,
...
SOURCE = (
    ...
    / "article"
    / "lc-160-intersection-of-two-linked-lists.md"
)

ARTICLE_TITLE = "【LC 160】相交链表：让 pA 走完 A 去走 B，两条路就一样长了"
ARTICLE_SLUG = "lc-160-intersection-of-two-linked-lists"
ARTICLE_EXCERPT = (
    "相交链表的最优解只有五行：pA 走完 A 就去走 B，pB 走完 B 就去走 A。"
    "本文从「相交判据是节点不是值」讲起，推出 a + c + b == b + c + a 这个必然同时到达的理由，"
    "并附可回放的 Y 字布局推演动画、完整推演表与六个常见坑。"
)
# 自制封面（tools/make_lc160_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-160-cover.png"
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc160"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-160-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
