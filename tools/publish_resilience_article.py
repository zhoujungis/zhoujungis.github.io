"""Publish the timeout/retry/circuit-breaker/degradation deep-dive article.

Usage:
    python tools/publish_resilience_article.py
    python tools/publish_resilience_article.py --cover https://...
...
ARTICLE_EXCERPT = (
    ...
    "重试 3 次把放大推到 x4.00 同时自身吞吐暴跌 88%，"
    "全抖动把恢复峰值从 1200 QPS 压到 520（-57%），"
    "重试预算让放大回到 x1.04 并带来 3.6 倍吞吐，"
    "熔断把拒绝率从 36.2% 降到 3.5%（代价是恢复探测慢 731ms），"
    "舱壁隔离让健康依赖 p50 从 436ms 回到 76ms。附完整参数清单与上线 Checklist。"
)
ARTICLE_COVER = (
...
TAGS = [
    ...
    {"slug": "circuit-breaker", "name": "熔断"},
    {"slug": "degradation", "name": "降级"},
