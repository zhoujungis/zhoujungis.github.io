"""Publish the LC 124 (binary tree maximum path sum) article — the twelfth algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc124_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc124_article.py
...
Source Markdown: ``article/lc-124-binary-tree-maximum-path-sum.md``.

Same guard set as publish_lc236_article.py (adapted, not shared, so each
...
- viz check: the placeholder must survive markdown + bleach —
  ``.algo-viz--lc124`` (tree with gain labels + best-path banner).
- number check: headline figures must be present in the text. The two-quantity
  split (``through`` for the answer vs ``gain`` for the report), the
  ``max(gain, 0)`` pruning, and the ``-Infinity`` initialisation are the spine
  of the article; if the prose and the state machine
  (frontend/src/viz/maxPathSumSteps.js) drift apart, this guard catches it.
...
SOURCE = (
    ...
    / "article"
    / "lc-124-binary-tree-maximum-path-sum.md"
)

ARTICLE_TITLE = "【LC 124】二叉树中的最大路径和：返回值为什么只能选一条胳膊"
ARTICLE_SLUG = "lc-124-binary-tree-maximum-path-sum"
ARTICLE_EXCERPT = (
    "LC 124 只有 10 行代码，但它逼你想清楚一件在别处很少被逼着面对的事："
    "同一个节点「向上汇报的值」和「用来更新答案的值」可以是两个不同的量 —— "
    "前者是一条胳膊（val + max(左, 右)），后者是两条胳膊的拱顶（val + 左 + 右）。"
    "本文说明为什么返回值只能选一条胳膊（路径不能拐两次弯）、"
    "为什么 max(gain, 0) 是算法的核心优化而不是防御性编程、"
    "为什么 best 必须初始化成 -Infinity。"
    "附可回放的双量分离推演动画、13 帧完整推演表，"
    "以及 543 / 687 / 1123 三个同骨架变体的对照。"
)
# 自制封面（tools/make_lc236_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-124-cover.png"
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc124"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-124-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
