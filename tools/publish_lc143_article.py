"""Publish the LC 143 (reorder list) article — the sixth algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc143_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc143_article.py
...
Source Markdown: ``article/lc-143-reorder-list.md``.
...
- viz check: the placeholder must survive markdown + bleach —
  ``.algo-viz--lc143`` (three-stage reorder walkthrough).
- number check: headline figures must be present in the text. The three-step
  ...
  implementation (frontend/src/viz/reorderSteps.js) drift apart, this guard
  is what catches it.
...
SOURCE = (
    ...
    / "article"
    / "lc-143-reorder-list.md"
)

ARTICLE_TITLE = "【LC 143】重排链表：一个题拆成三道做，中间那道你昨天刚刷过"
ARTICLE_SLUG = "lc-143-reorder-list"
ARTICLE_EXCERPT = (
    "重排链表 = 找中点 + 反转后半段 + 交替合并，三步分别是 LC 876、LC 206、LC 21 的子问题。"
    "本文说清中点为什么归前半段、断开那一行为什么是不成环的命门、合并循环为什么以 while second 为准，"
    "并附可回放的三阶段推演动画与边界推演表。"
)
# 自制封面（tools/make_lc143_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-143-cover.png"
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc143"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-143-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
