"""Publish the LC 3 (longest substring without repeating characters) article — the fourteenth algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc3_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc3_article.py
...
Source Markdown: ``article/lc-3-longest-substring-without-repeating-characters.md``.

First **string / sliding-window** entry in the series (everything before it was
linked lists and binary trees), so the animation is a new shape too: a row of
character cells + a window band + a lastSeen table.

Same guard set as publish_lc104_article.py (adapted, not shared, so each
...
- viz check: the placeholder must survive markdown + bleach —
  ``.algo-viz--lc3`` (string cells + sliding window + lastSeen table).
- number check: headline figures must be present in the text. The four spine
  claims are: the ``last[ch] >= left`` predicate (not just "char appeared"),
  the ``"abba"`` counter-example, the ``-1`` initialisation trap for the ASCII
  array version, and the "left only moves right" invariant. If the prose and
  the state machine (longestSubstrSteps.js) drift apart, this guard catches it.
...
SOURCE = (
    ...
    / "article"
    / "lc-3-longest-substring-without-repeating-characters.md"
)

ARTICLE_TITLE = "【LC 3】无重复字符的最长子串：判据不是「字符出现过」，而是「它还在窗口里」"
ARTICLE_SLUG = "lc-3-longest-substring-without-repeating-characters"
ARTICLE_EXCERPT = (
    "LC 3 是滑动窗口的入门模板，10 行代码，但在字节的面经里长期排第一 —— "
    "因为它有一个条件是漏了就必错的：判断该不该动左指针时，"
    "问的不是「这个字符见过吗」，而是「它上次出现的位置还在窗口里吗」。"
    "写成 `if ch in last` 会让左指针倒退，官方三个样例全部照样通过，"
    "而 `\"abba\"` 会当场把它打回原形（答案从 2 错成 3）。"
    "本文给出滑动窗口的两条不变量、逐帧推演动画、`\"abba\"` 的正确/错误两列对照表，"
    "以及 159 / 340 / 424 / 76 / 209 / 438 一整串变体的分岔点。"
)
# 自制封面（tools/make_lc236_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-3-cover.png"

CATEGORY = {"name": "算法", "slug": "algorithm", "description": "算法题解与推演"}

TAGS = [
    {"slug": "algorithm", "name": "算法"},
    {"slug": "sliding-window", "name": "滑动窗口"},
    {"slug": "string", "name": "字符串"},
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc3"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-3-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
