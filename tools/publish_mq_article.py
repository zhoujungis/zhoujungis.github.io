"""Publish the MQ reliable-delivery article.

Usage:
    python tools/publish_mq_article.py
    python tools/publish_mq_article.py --cover https://...
...
ARTICLE_EXCERPT = (
    ...
    "fire-and-forget 丢 615 条，只配生产端确认仍丢 488 条，全链路确认 + 手动 ACK + 幂等消费 0 丢失。"
    "含三跳丢失归因、本地消息表、指数退避实测、死信队列与上线 Checklist。"
)
ARTICLE_COVER = (
