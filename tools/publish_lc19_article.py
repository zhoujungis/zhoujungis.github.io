"""Publish the LC 19 (remove nth node from end of list) article — the fifth
algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc19_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc19_article.py
...
Source Markdown: ``article/lc-19-remove-nth-from-end.md``.

Same guard set as publish_lc141_142_article.py (adapted, not shared, so each
...
- viz check: the placeholder must survive markdown + bleach —
  ``.algo-viz--lc19`` (dummy + fast/slow one-pass walkthrough).
  The widget is mounted client-side by class name, so a stripped class means
  a silently missing animation.
- number check: headline figures must be present in the text. The ``n+1``
  convention is the core of the article; if the prose and the implementation
  (frontend/src/viz/removeNthSteps.js) drift apart, this guard is what
  catches it.
...
SOURCE = (
    ...
    / "article"
    / "lc-19-remove-nth-from-end.md"
)

ARTICLE_TITLE = (
    "【LC 19】删除链表的倒数第 N 个结点：一趟扫描的关键，是让快指针先出发"
)
ARTICLE_SLUG = "lc-19-remove-nth-from-end"
ARTICLE_EXCERPT = (
    "单链表不知道自己的长度，「倒数第 n 个」怎么在一趟扫描里定位？"
    "答案是让快指针先走 n+1 步、把间隔锁死，fast 落到 null 时 slow 恰好停在"
    "待删节点的前驱上。本文说清为什么是 n+1 而不是 n、哑结点怎么把「删头」"
    "变成普通删除，并附可回放的推演动画与边界推演表。"
)
# 自制封面（tools/make_lc19_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-19-cover.png"
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc19"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-19-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
