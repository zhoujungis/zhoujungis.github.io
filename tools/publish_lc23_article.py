"""Publish the LC 23 (merge k sorted lists) article — the third algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc23_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc23_article.py
...
Source Markdown: ``article/lc-23-merge-k-sorted-lists.md``.

Same guard set as publish_lc21_article.py (adapted, not shared, so each
...
- viz check: **two** placeholders must survive markdown + bleach —
  ``.algo-viz--lc23`` (min-heap) and ``.algo-viz--lc23dc`` (divide & conquer).
...
- number check: headline figures must be present in the text. The counts here
  come from ``tools/count_lc23_work.py`` — if the prose and the script drift
...
ARTICLE_TITLE = "【LC 23】合并 K 个升序链表：堆里到底该放几个节点"
ARTICLE_SLUG = "lc-23-merge-k-sorted-lists"
ARTICLE_EXCERPT = (
    "顺序合并 O(k²N) 的那笔账到底怎么来的？最小堆里为什么只放 K 个节点、"
    "不能放 N 个？两张可回放的推演动画（小顶堆 / 分治归并树），一份用"
    "tools/count_lc23_work.py 跑出来的精确比较次数对比表 —— 以及一个反直觉结论："
    "k 很小时堆反而是三种做法里最慢的。"
)
# 自制封面：动画截图（step 4，堆满 4 个候选 + 结果行已接 0..3），
# 由 frontend/preview-cover.html 生成。仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-23-cover.png"
...
# 后四个数字与 tools/count_lc23_work.py 的输出逐字对应 —— 改了脚本就要改正文，
...
# 两个动画占位符都必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc23", "algo-viz--lc23dc"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (frontend/public/photos/lc-23-cover.png),\n"
