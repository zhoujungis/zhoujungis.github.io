"""Publish the LC 206 (reverse linked list) article — the first「算法解读」entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc206_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc206_article.py
...
Source Markdown: ``article/lc-206-reverse-linked-list.md``.

Why this script carries a ``check_viz`` guard
─────────────────────────────────────────────
The article embeds an interactive animation via a bare placeholder:

    <div class="algo-viz algo-viz--lc206"></div>

The animation itself is built client-side, because the backend allowlist in
``articles/models.py`` has no ``svg`` / ``style`` / ``script`` and keeps only
``class`` on a ``div``. So the placeholder is the *entire* contract between the
article and the widget.

If bleach ever drops that class, nothing errors — the page renders fine, just
without the animation, and the article silently loses its centrepiece. That is
the same class of failure as the KaTeX ``delimiters`` bug: a guard that does not
run looks exactly like a guard that passed. Hence: render the markdown through
the backend's exact pipeline and assert the placeholder survived, plus assert
the code blocks came out unpolluted.
"""
...
ARTICLE_TITLE = "【LC 206】反转链表：指针一改就断链，问题到底出在哪"
ARTICLE_SLUG = "lc-206-reverse-linked-list"
ARTICLE_EXCERPT = (
    "一道标着「简单」却几乎每家都考的题。从「先记住 next、再掉头、后前移」"
    "这三步讲起，配 16 步可回放的推演动画；再拆递归的调用栈，最后连考 LC 25 "
    "「K 个一组反转」。"
)
# 自制封面：动画截图，由 frontend/preview-cover.html 生成。
# 它是仓库里的文件（frontend/public/photos/），必须先部署上去这张图才可访问 ——
# 所以发布顺序是「先部署、再发布」。见 cover_is_live()。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-206-cover.png"
...
EXPECTED_FIGURES = ["LC 206", "LC 25", "LC 92", "LC 24", "16 步", "O(1)", "O(n)"]

VIZ_PLACEHOLDER_CLASS = "algo-viz--lc206"
...
def check_viz(content):
    ...
    This is the guard that matters for this article: the widget is mounted by
    class name, so a stripped class means a silently missing animation.
    """
    import bleach
    import markdown
    ...
    if VIZ_PLACEHOLDER_CLASS not in cleaned:
        raise SystemExit(
            f"Animation placeholder `.algo-viz--lc206` did NOT survive the pipeline.\n"
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (frontend/public/photos/lc-206-cover.png),\n"
