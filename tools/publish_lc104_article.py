"""Publish the LC 104 + LC 226 (max depth & invert binary tree) article — the thirteenth algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc104_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc104_article.py
...
Source Markdown: ``article/lc-104-226-max-depth-and-invert-binary-tree.md``.

This is a **merged** entry (two easy problems in one post), same as the
LC 141/142 and LC 23/23dc pairs — the two share one skeleton (tree recursion)
but sit on opposite sides of "does the parent need the children's answers?".

Same guard set as publish_lc124_article.py (adapted, not shared, so each
...
- viz check: BOTH placeholders must survive markdown + bleach —
  ``.algo-viz--lc104`` (height bubbling up) and ``.algo-viz--lc226``
  (nodes physically swapping sides).
- number check: headline figures must be present in the text. The three spine
  claims are: 111's "空子树不是叶子" pitfall, 226's "交换不能夹在两个递归中间",
  and the "path bitwise flip / self-inverse" equivalence. If the prose and the
  state machines (maxDepthSteps.js / invertTreeSteps.js) drift apart, this
  guard catches it.
...
SOURCE = (
    ...
    / "article"
    / "lc-104-226-max-depth-and-invert-binary-tree.md"
)

ARTICLE_TITLE = "【LC 104 / 226】二叉树的最大深度 & 翻转二叉树：两道送分题里各藏着一颗钉子"
ARTICLE_SLUG = "lc-104-226-max-depth-and-invert-binary-tree"
ARTICLE_EXCERPT = (
    "104 和 226 都是 LeetCode 上的 easy，但它们恰好是树递归的两种模式，"
    "各自藏着一颗钉子。104（美团面经里的「求二叉树的高度」）的钉子长在 111 上 —— "
    "「空子树不是叶子」这句话，max 侥幸掩盖了它，min 一下就露馅："
    "`[1,2]` 会错答 1，正确答案是 2。"
    "226（Max Howell 那道著名的白板题）的钉子长在「交换的位置」上 —— "
    "顺序其实无所谓，唯独不能把交换夹在两个递归中间。"
    "附两段可回放的推演动画（高度自底向上冒 / 节点真的换位置）、"
    "23 帧完整推演表，以及 111 / 110 / 543 / 101 / 951 一整串变体的对照。"
)
# 自制封面（tools/make_lc236_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-104-cover.png"

CATEGORY = {"name": "算法", "slug": "algorithm", "description": "算法题解与推演"}

TAGS = [
    {"slug": "algorithm", "name": "算法"},
    {"slug": "binary-tree", "name": "二叉树"},
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc104", "algo-viz--lc226"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-104-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
