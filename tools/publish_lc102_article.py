"""Publish the LC 102 (binary tree level order traversal) article — the tenth algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc102_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc102_article.py
...
Source Markdown: ``article/lc-102-binary-tree-level-order-traversal.md``.
...
- viz check: the placeholder must survive markdown + bleach —
  ``.algo-viz--lc102`` (tree + queue bar + level-bracket walkthrough).
- number check: headline figures must be present in the text. ``size = len(q)``
  and the ``1 / 2 / 2`` level sizes are the spine of the article; if the prose
  and the implementation (frontend/src/viz/levelOrderSteps.js) drift apart,
...
ARTICLE_TITLE = "【LC 102】二叉树的层序遍历：为什么必须先把队列长度存下来"
ARTICLE_SLUG = "lc-102-binary-tree-level-order-traversal"
ARTICLE_EXCERPT = (
    "层序遍历的代码只有十行，但那个 size = len(queue) 的快照是整题的唯一难点。"
    "本文从「队列里本来就有分层」讲起，说清为什么必须存快照而不能用实时长度，"
    "并一口气带出 103 锯齿、199 右视图、107 自底向上三个变体的改法，"
    "附可回放的树 + 队列 + 结果区三段式推演动画与完整推演表。"
)
# 自制封面（tools/make_lc102_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-102-cover.png"
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc102"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-102-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
