"""Publish the LC 1 (two sum) article — the fifteenth algorithm entry.

Usage:
    backend/venv/Scripts/python.exe tools/publish_lc1_article.py --dry-run
    backend/venv/Scripts/python.exe tools/publish_lc1_article.py
...
Source Markdown: ``article/lc-1-two-sum.md``.

Same guard set as publish_lc3_article.py (adapted, not shared, so each
...
- viz check: the placeholder must survive markdown + bleach —
  ``.algo-viz--lc1`` (array cells + i pointer + seen hash table + answer arc).
- number check: headline figures must be present in the text. The spine claims
  are: the "先查再存" ordering rule, the exact trigger condition
  (``target == 2 * x``), the fact that the FIRST official sample
  ``[2, 7, 11, 15] / 9`` happens to MASK the bug, and the LC 167 双指针 contrast.
...
ARTICLE_TITLE = "【LC 1】两数之和：「先查再存」的顺序一反过来，就会匹配到自己"
ARTICLE_SLUG = "lc-1-two-sum"
ARTICLE_EXCERPT = (
    "LC 1 是绝大多数人的第一道 LeetCode，但它真正教的东西只有两点："
    "把「枚举组合」换成「查表」，以及两句不能颠倒的代码 —— "
    "先问「前面有我缺的那个数吗」，再把自己登记进去。"
    "顺序反过来的话，当 `target == 2 * x` 时会查到**自己**，返回 [i, i]，"
    "把同一个元素用了两次。更阴的是：官方给的第一组样例 `[2,7,11,15]` 配 `9` "
    "恰好不触发这个 bug（9 的一半不是整数），跑通它根本不代表写对了。"
    "本文给出逐帧推演动画、`[3,3]` 的正确/错误两列对照表、"
    "以及 LC 167 双指针 / 15 三数之和 / 454 分组哈希的演化路线。"
)
# 自制封面（tools/make_lc236_cover.py 生成），仓库文件，必须先部署才可访问。
ARTICLE_COVER = "https://zhoujungis.github.io/photos/lc-1-cover.png"
...
# 动画占位符必须活下来
VIZ_PLACEHOLDER_CLASSES = ["algo-viz--lc1"]
...
def main():
    ...
    if not args.skip_cover_check and not cover_is_live(args.cover):
        raise SystemExit(
            ...
            "  This cover is a repo file (photos/lc-1-cover.png),\n"
            "  so it only becomes reachable AFTER the deploy that ships it.\n"
